...
var default_itemID = "LVTN_0.21/0.03x7_Vtsat";
...
$(document).ready(function(){
	...
	d3.csv("../RAWDATA/28nmET_CIS.csv", type_CIS, function(error, data) {
		if (error) {
			console.log(error);
			throw error;
		} else {
			...
			d3.select("#itemidoption").selectAll("g")
				.append('option').data(d3.keys(data[0]).filter(function (d) {
					return (d != "LOTID5" && d != "WAFER_ID" && d != "time" && d != "PROCESS_ID" && d != "PROCESS_ID_LE821910_PROCESS_ID_MAX" && d != "ET_TIME_LE821910_TKOUT_TIME_MAX") ? d : null;}))
				.enter().append('option')
		...
		width = $("#et-data").innerWidth() - margin.left - margin.right;
		height = 500 - margin.top - margin.bottom;
		x = d3.scale.ordinal().rangePoints([0, width], 5);
	...
	$("#testlayeroption").change(function(){
		if($(this).find("option:selected").attr("value")=="MF"){
			...
			d3.csv("../RAWDATA/28nmET_CIS_inline.csv", type_CIS_inline, function(error, data) {
				if (error) {
					console.log(error);
					throw error;
				} else {
					...
					d3.select("#itemidoption").selectAll("g")
						.append('option').data(d3.keys(data[0]).filter(function (d) {
							return (d != "LOTID5" && d != "WAFER_ID" && d != "time" && d != "PROCESS_ID" && d != "PROCESS_ID_LE522600_PROCESS_ID_MAX" && d != "ET_TIME_LE522600_TKOUT_TIME_MAX") ? d : null;}))
						.append('option')
						.text(function(d){return d;})
						.attr("value", function(d){return d;});
				}
	...
	if (layer == "MF" && device != null && option != null) {
		...
		d3.csv("../RAWDATA/28nmET_CIS_inline.csv", type_CIS_inline, function (error, data) {
			if (error) {
				...
				var join_data = leftsvg.selectAll(".dot").data(data.filter(function(d){return (device == "ALL" || device == d.PROCESS_ID)  ? d.time: null;}), function(d){return d.date_lot.concat(d.WAFER_ID);});
				//exit() selection
				join_data.exit().transition().ease("cubicInOut").duration(600).style("opacity",0).remove();
				/*d3.transition().duration(600).each(function(){
					join_data.exit()
						.transition().style("opacity",0).remove();
				});*/
...
function type_CIS(d) {
	//d.time = d3.time.format("%Y-%m-%d %H:%M:%S").parse(d.ET_TIME_LE821910_TKOUT_TIME_MAX);
	d.time = d.date_lot;
	d.PROCESS_ID = d.PROCESS_ID_LE821910_PROCESS_ID_MAX;
...
function type_CIS_inline(d) {
	...
	d.PROCESS_ID = d.PROCESS_ID_LE522600_PROCESS_ID_MAX;
	return d;
}
