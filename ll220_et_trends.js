...
var default_itemID = "HVTN_0.21/0.03x7_Vtsat";
...
$(document).ready(function(){
		...
		if (error) {
			...
			d3.select("#itemidoption").selectAll("g")
				.append('option').data(d3.keys(data[0]).filter(function (d) {
					return (d != "LOTID5" && d != "WAFER_ID" && d != "time" && d != "PROCESS_ID" && d != "PROCESS_ID_LX856900_PROCESS_ID_MAX" && d != "ET_TIME_LX856900_TKOUT_TIME_MAX") ? d : null;}))
				.enter().append('option')
	...
	$("#testlayeroption").change(function(){
		if($(this).find("option:selected").attr("value")=="MF"){
			...
			d3.csv("../RAWDATA/28nmET_DDI_inline.csv", type_DDI_inline, function(error, data) {
				if (error) {
					console.log(error);
					throw error;
				} else {
					...
					d3.select("#itemidoption").selectAll("g")
						.append('option').data(d3.keys(data[0]).filter(function (d) {
							return (d != "LOTID5" && d != "WAFER_ID" && d != "time" && d != "PROCESS_ID" && d != "PROCESS_ID_LX562150_PROCESS_ID_MAX" && d != "ET_TIME_LX562150_TKOUT_TIME_MAX") ? d : null;}))
						.append('option')
						.text(function(d){return d;})
						.attr("value", function(d){return d;});
				}
...
function type_DDI(d) {
	...
	d.PROCESS_ID = d.PROCESS_ID_LX856900_PROCESS_ID_MAX;
...
function type_DDI_inline(d) {
	//d.time = d3.time.format("%Y-%m-%d %H:%M:%S").parse(d.ET_TIME_LX562150_TKOUT_TIME_MAX);
	d.time = d.date_lot;
	d3.keys(d).forEach(function(orig){	//Real numberify for all the rest of the columns, orig is col header name
		if(orig != "time" || orig != "PROCESS_ID")
			orig.value = +orig.value;
	});
	d.PROCESS_ID = d.PROCESS_ID_LX562150_PROCESS_ID_MAX;
	return d;
}
function type_CIS(d) {
